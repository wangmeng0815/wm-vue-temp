...
config.HTMLDirs.forEach((item) => {
  ...
  let pageHtml = `src/views/${item.page}/index.html`;
  ...
  const htmlPlugin = new HTMLWebpackPlugin({
    ...
    chunks: [item.page, 'vendors'] // html文件中需要要引入的 js模块，这里的 vendor 是 webpack 默认配置下抽离的公共模块的名称
  });
  ...
  Entries[item.page] = [resolve(`src/views/${item.page}/index.js`)]; // 根据配置设置入口js文件
});
